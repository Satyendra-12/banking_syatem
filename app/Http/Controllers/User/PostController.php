...
class PostController extends Controller
{
 public function listPage()
    {
        $get = Listing::get();
        return view('user.list.list', compact('get'));
    }

    public function applistPage()
    {
        $get = Listing::get();
        return view('user.list.applist', compact('get'));
    }
    public function rejlistPage()
    {
        $get = Listing::get();
        return view('user.list.rejlist', compact('get'));
    }
    public function addlist()
    {
        ...
        return view('user.list.add_list',compact('state','category'));
    }
    ...
    public function savelist(Request $request)
    {
        ...
        // $image = $request->file('list_cover');
        // $ext = $image->getClientOriginalExtension();
        // $name = substr(uniqid(), 0, 9) . '.' . $ext;
        // $profile = '/storage/app/public/list/image' . $name;
        // $image->save(storage_path('storage/app/public/list/image'));
        ...
        // $image->save(storage_path('app/public/list/image'));
        // $request->file('list_cover')->move(storage_path() . '/app/public/list/icon', $name);
        // }

        // if ($request->hasfile('list_icon'))
        // {
        //     $ext = $request->file('list_icon')->getClientOriginalExtension();
        //     $name = substr(uniqid(), 0, 9) . '.' . $ext;
        //     $iconprofile = 'storage/app/public/list/icon/' . $name;
        //     $request->file('list_icon')->move(storage_path() . '/app/public/list/icon/', $name);
        //     $list->list_icon = $iconprofile;
        // }

        $saved = $list->save();

        if ($saved) {
            return response()->json(array('status' => true, 'msg' => 'Successfully Added', 'location' => route('user.listPage')));
        } else {
    ...
    public function editlist($id)
    {
        ...
        return view('user.list.list_edit', compact('edit','state','category','subcategory'));
    }
  ...
  $update->update();
        ...
        if ($update) {
            return response()->json(array('status' => true, 'msg' => 'Successfully Updated', 'location' => route('user.listPage')));
        } else {
            return response()->json(array('status' => false, 'msg' => 'Something Went wrong,please try again'));
        }
    }
    ...
    public function deletelist(Request $request)
    {
        ...
        if ($delete) {
            return response()->json(array('status' => true, 'msg' => 'Successfully Delete', 'location' => route('user.listPage')));
        }
    }
    ...
    public function listAjaxList(Request $request)
    {
        ...
        ->join('subcategories', 'listings.subcategory_id', '=', 'subcategories.id')
            ->orWhere(function ($query) use ($search) {
                ...
                $query->orWhere('subcategories.sub_category_name', 'like', '%' . $search . '%');
            });
        ...
        foreach ($lists as $list) {
            ...
            $status = '<div>
            <button  type="button" class="btn btn-secondary dropdown-toggle ' . $color . ' " data-id="' . $list->id . '" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">' . ($list->status == 1 ? "Active" : "Inactive") . ' </button>
            ...
            $data[] = array(
                ...
                $status,
                '<a href="' . route('user.editlist', $list->id) . '" class="h5 p-0 pl-2  edit_list"><i class="mdi mdi-account-edit"></i></a>
