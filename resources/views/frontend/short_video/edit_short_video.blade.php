            ...
            <div class="breadcrumb-wrapper breadcrumb-contacts">
                <div>
                    ...
                    <p class="breadcrumb-item mt-3">
                        ...
                        <span><a href="{{ route('user.shortVideo') }}"><i class="mdi mdi-chevron-right"> {{ __('lang.short')}}</i></a></span>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="ec-vendor-list card card-default">
                        ...
                        <div class="card-body">
                            <form id="updateshortvideo" enctype="multipart/form-data">
                                <div class="row">

                                    <div class=" col-6 form-group">
                                        <label for=" Titel">{{ __('lang.title')}}</label>
                                        <input type="hidden" name="id" value="{{ $editshortvideo->id }}">
                                        <input type="text" class="form-control" name="title"
                                            value="{{ $editshortvideo->title }}" placeholder="{{ __('lang.enter_title_name')}}">
                                    </div>

                                    <div class=" col-6 form-group">
                                        <label for=" Video">{{ __('lang.video')}}</label>
                                        <input type="file" class="form-control" name="video">
                                        <video src="{{ asset($editshortvideo->video) }}" width="150px" height="150px"
                                            class="mt-3"></video>
...
@section('extra_js')
    <script>
        $(function() {
            $('#updateshortvideo').on('submit', function(e) {
                //    alert('hello');
                e.preventDefault()
                let fd = new FormData(this)
                fd.append('_token', "{{ csrf_token() }}");
                $.ajax({
                    url: "{{ route('user.usereditupdate') }}",
                    type: "POST",
                    data: fd,
                    dataType: 'json',
                    processData: false,
